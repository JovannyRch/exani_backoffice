...
export default function EditQuestionPage() {
  ...
  const { result: question, query } = useOne({
    ...
    meta: {
      select:
        "id, stem, explanation, difficulty, skill_id, set_id, source, tags_json, options_json, correct_key, is_active",
    },
  });
  ...
  // Fetch skill details to get hierarchy
  const { result: skillDetailResult } = useList({
    resource: "skills",
    filters: question?.skill_id
      ? [{ field: "id", operator: "eq", value: question.skill_id }]
      : [],
    meta: { select: "id, area_id" },
    queryOptions: { enabled: !!question?.skill_id },
  });

  const { result: areaDetailResult } = useList({
    resource: "areas",
    filters: skillDetailResult?.data?.[0]?.area_id
      ? [
          {
            field: "id",
            operator: "eq",
            value: skillDetailResult.data[0].area_id,
          },
        ]
      : [],
    meta: { select: "id, section_id" },
    queryOptions: { enabled: !!skillDetailResult?.data?.[0]?.area_id },
  });

  const { result: sectionDetailResult } = useList({
    resource: "sections",
    filters: areaDetailResult?.data?.[0]?.section_id
      ? [
          {
            field: "id",
            operator: "eq",
            value: areaDetailResult.data[0].section_id,
          },
        ]
      : [],
    meta: { select: "id, exam_id" },
    queryOptions: { enabled: !!areaDetailResult?.data?.[0]?.section_id },
  });
  ...
  useEffect(() => {
    if (question) {
      reset({
        ...
        set_id: question.set_id?.toString() || "",
      ...
      if (question.options_json) {
        ...
        if (Array.isArray(question.options_json)) {
          loadedOptions = question.options_json;
        }
        // Si es un objeto (formato clave-valor)
        else if (typeof question.options_json === "object") {
          loadedOptions = Object.entries(question.options_json).map(
            ([key, text]) => ({
              key,
              text: String(text),
            }),
      ...
      setCorrectKey(question.correct_key || "");
    }
  }, [question, reset]);

  // Set hierarchy when skill data loads
  useEffect(() => {
    if (sectionDetailResult?.data?.[0]?.exam_id) {
      setExamId(sectionDetailResult.data[0].exam_id.toString());
    }
  }, [sectionDetailResult]);

  useEffect(() => {
    if (areaDetailResult?.data?.[0]?.section_id) {
      setSectionId(areaDetailResult.data[0].section_id.toString());
    }
  }, [areaDetailResult]);

  useEffect(() => {
    if (skillDetailResult?.data?.[0]?.area_id) {
      setAreaId(skillDetailResult.data[0].area_id.toString());
    }
  }, [skillDetailResult]);
              ...
              <CardContent className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={(v) =>
                      setValue("skill_id", v, { shouldDirty: true })
                    }
                    disabled={!areaId}
                  >
                    <SelectTrigger>
                ...
                <div className="space-y-1.5">
                  ...
                  <Select
                    value={watch("set_id")}
                    onValueChange={(v) =>
                      setValue("set_id", v, { shouldDirty: true })
                    }
                    disabled={!examId}
                  >
                    <SelectTrigger>
